...
- **Layout is three flex columns, not two:** `[thumbnail + title, wrapped in one Link]` → `[status badge column]` → `[actions column]`. The status badge must NOT live inside the navigation `<Link>` — `LibraryItemCard` already hit this exact problem (a clickable badge nested inside a navigation link either fires both behaviors or requires `preventDefault`/`stopPropagation` workarounds) and solved it by extracting the badge to a sibling of the `Link`. The row variant follows the same precedent: the `Link` wraps only the thumbnail (`.series-list-thumb`) and title (`.series-list-title`, reused as-is — both are standalone classes, not scoped to a parent selector, so they apply correctly here too); the status badge sits in its own `.library-row-status-wrapper` (`position: relative`, `display: inline-block`) as a sibling of the `Link`, between it and the actions column. The status badge keeps the exact same click-to-open dropdown behavior and PATCH call as `LibraryItemCard` (`handleStatusChange`) — the existing `.library-status-menu` (`position: absolute; top: calc(100% + ...)`) still anchors correctly relative to its own trigger wrapper regardless of where that wrapper sits in the row.
- **Right side:** the same `+1 {label} ({value})` progress button and remove-with-confirm control as `LibraryItemCard`, in a `.library-row-actions` flex cluster. The `Link` column gets `flex: 1; min-width: 0` so it absorbs all extra space, naturally pushing the status badge and actions columns to the row's right edge without needing `margin-left: auto` on either.
...
- `.library-list` — flex column container for the row list (`display: flex; flex-direction: column; gap: var(--space-2);` — same rules as `.series-list`, defined under its own class name in case Library-specific row sizing needs to diverge later).
- `.library-list-row` — the outer row: `display: flex; align-items: center; gap: var(--space-3);` plus the card-like border/background/padding `.series-list-row` already uses, containing exactly three children: the `Link`, the status wrapper, and the actions cluster.
- `.library-list-row-link` — the `Link`'s own styling: `display: flex; align-items: center; gap: var(--space-3); flex: 1; min-width: 0; text-decoration: none; color: inherit;` (mirrors `.series-list-row`'s own flex/text-decoration rules, renamed to avoid colliding with that unrelated class). `flex: 1` makes it absorb all extra space, pushing the status badge and actions to the row's right edge.
- `.library-row-status-wrapper` — `position: relative; display: inline-block; flex-shrink: 0;` (the inline-flow counterpart to `.library-status-wrapper`'s absolute positioning).
- `.library-row-actions` — `display: flex; align-items: center; gap: var(--space-2); flex-shrink: 0;`.
