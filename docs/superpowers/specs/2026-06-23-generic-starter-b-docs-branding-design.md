...
5. **`docs/architecture.md`** — **full rewrite, not a genericization.** Direct inspection found this doc (and design-patterns.md, project-structure.md) documents a *fictional* target architecture — Repository pattern, Service layer, Strategy-pattern Providers, an AI recommendation engine (§3.1, Phase 2.7), social features (follow/feed, Phase 3.2/3.5) — none of which were ever built. The real codebase (confirmed throughout A1-A3) has API routes calling Prisma directly via `src/lib/db/prisma.ts`, no repository/service/provider layers, no AI, no social features; `CLAUDE.md` has disclaimed this gap since before the pivot ("No service/repository/provider layers exist... routes call Prisma directly"). Decision: **rewrite to document the real, actually-built architecture** — routes → Prisma direct, the HOF middleware chain (`withErrorHandler`/`withRateLimit`/`compose` in `src/lib/utils/middleware.ts`), Zod validation, the real `Item`/`UserItem` data flow — dropping the fictional layers entirely rather than keeping them as an aspirational target. More honest for a portfolio piece: a reader who opens the actual code after reading these docs should find what they were told to expect.

6. **`docs/project-structure.md`** (Turkish) — same full-rewrite treatment as architecture.md, for the same reason (documents `repositories/`, `services/`, `providers/`, `swagger/`, `(auth)`/`(main)` route groups, `social/`/`users/[id]/badges` API routes — none of which exist; the actual `src/app` structure is flat, per `CLAUDE.md`). Rewrite to the real, current file tree (confirmed by reading the actual directory structure directly, not the old doc) and the real conventions actually followed (kebab-case files, PascalCase components, `interface Props {}`, Zod validation, the route handler `compose(withErrorHandler, withRateLimit)(handler)` pattern).
